...
Figure 1 compares the two shrinkage priors that are the focus of our study. Both priors share a large peak at zero, which ensures that cross loadings are shrunken towards zero. However, the RHSP has much thicker tails. Here, for larger cross loadings, there is thus much more prior mass than with the SVNP. This ensures large cross loadings (and consequently other model parameters) can be estimated without bias within a single estimation step. 
